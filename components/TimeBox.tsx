...
const TimeBox: React.FC<TimeBoxProps> = ({ 
  ...
  const handleSlotMouseEnter = (hour: number, slot: 1 | 2) => {
    ...
    // Optimization: Only update state if the selection actually changed
    // This prevents infinite re-render loops/freezing during mouse moves (e.g., text selection)
    const prev = selectedSlots.join(',');
    const next = newRange.join(',');
...
// Memoize Row to prevent unnecessary re-renders of all rows when typing in one
const TimeBoxRow = React.memo(({
  hour,
  data,
  handleSlotMouseDown,
...
const OptimizedInput: React.FC<{ initialValue: string, onUpdate: (val: string) => void }> = React.memo(({ initialValue, onUpdate }) => {
